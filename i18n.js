(()=>{
  ...
  function apply(){
    ...
    document.title=lang==='ru'?'Луговой 1':lang==='en'?'Lugovoy 1':lang==='fr'?'Lugovoy 1':'Lugovoy 1';
    ...
    const selectionTitle=document.querySelector('.selection-title h2');if(selectionTitle){const count=document.getElementById('count')?.textContent||'0';selectionTitle.innerHTML=`${t.selected} <span id="count">${count}</span>`;}
    ...
    document.querySelectorAll('#spring-detail-dialog .service-contact-link').forEach(el=>{if(el.textContent.includes('сайт')||el.textContent.includes('website')||el.textContent.includes('site'))el.textContent=`🌐 ${t.openSite}`;});
    updateSwitcher();
  }

  function updateSwitcher(){
    const btn=document.querySelector('.lang-current');if(!btn)return;
    const l=languages[lang];btn.innerHTML=`<span>${l.flag}</span><b>${l.code}</b>`;btn.setAttribute('aria-label',`${tr[lang].language}: ${l.name}`);
  ...
  const observer=new MutationObserver(()=>{clearTimeout(observer._t);observer._t=setTimeout(apply,30);});
  const start=()=>{ensureSwitcher();apply();observer.observe(document.body,{childList:true,subtree:true});};
